...
{    ## no critic (RequireInterpolationOfMetachars)
  ...
  if (not @fps) {
    croak "Error: No output files created in '$out_dir'";
  }
  my $count = @fps;
  if ($count > 1) {
...
sub _read_file ($self, $fp) {   ## no critic (RequireInterpolationOfMetachars)
  ...
  if (not -r $fp) {
    croak "Error: Cannot read file '$fp'";
  }
...
{    ## no critic (RequireInterpolationOfMetachars)
  ...
  if (-r $fp) {
    croak "Error: Output file '$fp' already exists";
  }
  ...
  if (not -r $fp) {
    croak "Error: Unable to write '$fp'";
  }
